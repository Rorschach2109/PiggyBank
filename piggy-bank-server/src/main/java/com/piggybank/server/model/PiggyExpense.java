...
public class PiggyExpense implements java.io.Serializable, Comparable<PiggyExpense> {
	...
	@Override
	public int hashCode() {
		return Objects.hash(this.title.toLowerCase(), this.date, 
	...
	public boolean equals(Object object) {
		...
		return this.title.toLowerCase().equals(expense.title.toLowerCase()) &&
	...
	public int compareTo(PiggyExpense expense) {
		...
		compareResult = this.category.compareTo(expense.category);
		if (0 != compareResult) {
			return compareResult;
		}
		
		compareResult = this.title.toLowerCase().compareTo(expense.title.toLowerCase());
		if (0 != compareResult) {
			return compareResult;
		}
		
		return Double.valueOf(this.amount).compareTo(expense.amount);
	}
}
